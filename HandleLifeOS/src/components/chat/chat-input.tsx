...
export function ChatInput({ value, onChange, onSubmit, onStop, isLoading, disabled, voiceLanguage }: ChatInputProps) {
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) {
      ...
      if (value.trim() && !isLoading && !disabled) {
        onSubmit(e as unknown as { preventDefault: () => void })
      }
    }
  }
  ...
  const handleVoiceFinal = (text: string) => {
    onChange(text)
    // Auto-submit after a short tick so state updates first
    setTimeout(() => {
      const fakeEvent = { preventDefault: () => {} }
      if (text.trim()) onSubmit(fakeEvent)
    }, 50)
  }

  return (
    <form onSubmit={onSubmit} className="relative flex items-end gap-2">
      {/* Voice mic button — left of the textarea */}
      <VoiceMicButton
